    ...
    <div class="col-md-12 col-sm-12 col-xs-12">
      ...
      <div class="x_panel">
        ...
        <div class="x_title">
          ...
          <h2>{{trans('report.Report')}}: {{trans('report.Titlej')}} </h2>
        ...
        <div class="x_content">
          <!--<button id="exportButton" class="btn btn-lg btn-danger clearfix"><span class="fa fa-file-pdf-o"></span> Export to PDF</button>-->
              ...
              <table id="datatable">
                  ...
                  <tbody>
                    <tr>
                      <th>{{trans('report.Technical')}}</th>
                      <td>{{$mpgtec+$mbatec+$mdiptec+$msectec+$mlsectec}}</td>
                      <td>{{$fpgtec+$fbatec+$fdiptec+$fsectec+$flsectec}}</td>
                    </tr>
                      <tr>
                          <th>{{trans('report.Administrative')}}</th>
                          <td>{{$mpgad+$mbaad+$mdipad+$msecad+$mlsecad}}</td>
                          <td>{{$fpgad+$fbaad+$fdipad+$fsecad+$flsecad}}</td>
                      </tr>
                      <tr>
                        <th>{{trans('report.TrainingCoordinator')}}</th>
                        <td>{{$mpgtc+$mbatc+$mdiptc+$msectc+$mlsectc}}</td>
                        <td>{{$fpgtc+$fbatc+$fdiptc+$fsectc+$flsectc}}</td>
                      </tr>
                      <tr>
                          <th>{{trans('report.TrainingOfficer')}}</th>
                          <td>{{$mpgto+$mbato+$mdipto+$msecto+$mlsecto}}</td>
                          <td>{{$fpgto+$fbato+$fdipto+$fsecto+$flsecto}}</td>
                      </tr>
                      <tr>
                          <th>{{trans('report.Trainer')}}</th>
                          <td>{{$mpgtr+$mbatr+$mdiptr+$msectr+$mlsectr}}</td>
                          <td>{{$fpgtr+$fbatr+$fdiptr+$fsectr+$flsectr}}</td>
                      </tr>
                      <tr>
                        <th>{{trans('report.Teacher')}}</th>
                        <td>{{$mpgte+$mbate+$mdipte+$msecte+$mlsecte}}</td>
                        <td>{{$fpgte+$fbate+$fdipte+$fsecte+$flsecte}}</td>
                      </tr>
                      <tr>
                          <th>{{trans('report.User')}}</th>
                          <td>{{$mpgus+$mbaus+$mdipus+$msecus+$mlsecus}}</td>
                          <td>{{$fpgus+$fbaus+$fdipus+$fsecus+$flsecus}}</td>
                      </tr>
